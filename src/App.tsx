    ...
    <div className="h-screen flex flex-col bg-black overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#121212]">
          {/* Topbar */}
          <header className="h-14 md:h-16 flex items-center px-3 md:px-4 gap-2 md:gap-4 bg-[#121212]/90 backdrop-blur-sm flex-shrink-0 z-10">
            {/* Nav buttons — solo en escritorio */}
            ...
            {/* Logo Spotifake — solo en móvil */}
            <div className="flex md:hidden items-center gap-2">
              <SpotifyLogo />
              <span className="text-white font-bold text-sm">Spotifake</span>
            </div>
            ...
            <div className="flex-1 md:max-w-sm">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white text-black text-sm rounded-full pl-9 pr-4 py-2 md:py-2.5 outline-none focus:ring-2 focus:ring-white/40 placeholder-gray-500 font-medium"
                />
              </div>
            </div>
            ...
            <div className="ml-auto flex items-center gap-2 md:gap-3">
              {/* "Subir música" — texto completo en escritorio, solo ícono en móvil */}
              <button
                onClick={handleUploadClick}
                className="flex items-center gap-2 bg-white text-black text-sm font-bold px-3 md:px-4 py-2 rounded-full hover:bg-gray-100 transition-colors"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 flex-shrink-0">
                  <path d="M11 11V3h2v8h8v2h-8v8h-2v-8H3v-2z" />
                </svg>
                <span className="hidden md:inline">Subir música</span>
              </button>

              <button className="w-8 h-8 md:w-9 md:h-9 bg-[#1DB954] rounded-full flex items-center justify-center hover:scale-105 transition-transform">
                <SpotifyLogo />
              </button>
            </div>
          </header>

          {/* View content — con padding inferior en móvil para la navbar */}
          <div className="flex-1 min-h-0 flex pb-0 md:pb-0">
      ...
      <nav className="flex md:hidden items-center justify-around bg-[#0a0a0a] border-t border-white/10 flex-shrink-0 z-20"
           style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        {/* Inicio */}
        <button
          onClick={() => navigateTo('home')}
          className={`flex flex-col items-center gap-1 py-2 px-4 transition-colors ${view === 'home' ? 'text-white' : 'text-[#b3b3b3]'}`}
        >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 py-2 px-4 transition-colors ${view === 'search' ? 'text-white' : 'text-[#b3b3b3]'}`}
        >
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
            <path d="M10.533 1.279c-5.18 0-9.407 4.927-9.407 11.166C1.126 18.61 5.353 23.537 10.533 23.537c2.507 0 4.787-1.194 6.512-3.102l3.974 3.305a.985.985 0 0 0 1.392-.169 1.12 1.12 0 0 0-.164-1.476l-3.976-3.308c1.085-1.617 1.706-3.623 1.706-5.842 0-6.24-4.226-11.166-9.444-11.166zm-7.178 11.166c0-5.063 3.221-9.001 7.178-9.001 3.978 0 7.222 3.938 7.222 9.001S14.512 21.372 10.533 21.372c-3.978 0-7.178-3.939-7.178-9.001z" />
          </svg>
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 py-2 px-4 transition-colors ${view === 'library' ? 'text-white' : 'text-[#b3b3b3]'}`}
        >
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
            <path d="M3 22a1 1 0 0 1-1-1V3a1 1 0 0 1 2 0v18a1 1 0 0 1-1 1zM15.5 2.134A1 1 0 0 0 14 3v18a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V6.464a1 1 0 0 0-.5-.866l-6-3.464zM9 2a1 1 0 0 0-1 1v18a1 1 0 1 0 2 0V3a1 1 0 0 0-1-1z" />
          </svg>
          <span className="text-[10px] font-medium">Tu biblioteca</span>
        </button>

        {/* Subir */}
        <button
          onClick={() => navigateTo('upload')}
          className={`flex flex-col items-center gap-1 py-2 px-4 transition-colors ${view === 'upload' ? 'text-white' : 'text-[#b3b3b3]'}`}
        >
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
            <path d="M11 11V3h2v8h8v2h-8v8h-2v-8H3v-2z" />
          </svg>
