        ...
        {tracks.length > 0 ? (
          <div>
            ...
            <div className="bg-[#181818] rounded-xl overflow-hidden">
              {/* Header */}
              <div className="grid grid-cols-[2rem_1fr_1fr_5rem_5rem_2rem] gap-4 px-6 py-3 border-b border-[#282828] text-[#b3b3b3] text-xs uppercase tracking-wider font-medium">
                <span>#</span>
                <span>Título</span>
                <span>Artista</span>
                <span>Duración</span>
              ...
              {tracks.map((track, idx) => {
                ...
                return (
                  <div
                    key={track.id}
                    className={`grid grid-cols-[2rem_1fr_1fr_5rem_5rem_2rem] gap-4 px-6 py-3 items-center group cursor-pointer transition-colors ${
                      isActive ? 'bg-[#282828]' : 'hover:bg-[#282828]/50'
                    }`}
                    onDoubleClick={() => playTrack(track)}
                    onClick={() => playTrack(track)}
                  >
                    <span className={`text-sm ${isActive ? 'text-[#1DB954]' : 'text-[#b3b3b3] group-hover:hidden'}`}>
                      {isActive ? (
